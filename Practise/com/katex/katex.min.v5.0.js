    ...
    return function h(e, t, i) {
        ...
        6: [function(e, t, i) {
            ...
            o.prototype.expect = function(e, t) {
                if (e.text !== t) {
                    throw new n("Expected '" + t + "', got '" + e.text + "'", this.lexer, e.position)
                }
            };
            ...
            o.prototype.parseInput = function(e, t) {
                var i = this.parseExpression(e, t, false);
                this.expect(i.peek, "EOF");
                return i
            };
            var u = ["}", "\\end", "\\right", "&", "\\\\", "\\cr"];
            o.prototype.parseExpression = function(e, t, i, h) {
                var a = [];
                var r = null;
                while (true) {
                    r = this.lexer.lex(e, t);
                    if (u.indexOf(r.text) !== -1) {
                        break
                    }
                    if (h && r.text === h) {
                        break
                ...
                var s = new g(this.handleInfixNodes(a, t), e);
                s.peek = r;
                return s
            };
            o.prototype.handleInfixNodes = function(e, t) {
                var i = -1;
                var a;
                var r;
                for (var l = 0; l < e.length; l++) {
                    ...
                    if (s.type === "infix") {
                        if (i !== -1) {
                            throw new n("only one infix operator per group", this.lexer, -1)
                        }
                        i = l;
            ...
            var w = 1;
            o.prototype.handleSupSubscript = function(e, t, i, a) {
                var r = this.parseGroup(e, t);
                if (!r) {
                    throw new n("Expected group after '" + i + "'", this.lexer, e)
                } else if (r.isFunction) {
                    var l = h.funcs[r.result.result].greediness;
                    if (l > w) {
                        return this.parseFunction(e, t)
                    } else {
                        throw new n("Got function '" + r.result.result + "' with no arguments " + "as " + a, this.lexer, e)
                    }
                } else {
                    return r.result
                }
            };
            o.prototype.parseAtom = function(e, t) {
                ...
                while (true) {
                    ...
                    if (s.text === "^") {
                        if (a) {
                            throw new n("Double superscript", this.lexer, h)
                        }
                        ...
                        if (r) {
                            throw new n("Double subscript", this.lexer, h)
                        }
                        l = this.handleSupSubscript(s.position, t, s.text, "subscript");
                        h = l.position;
                        r = l.result
                    } else if (s.text === "'") {
                        var p = new c("textord", "\\prime", t);
                        var o = [p];
                        h = s.position;
                        ...
                        a = new c("ordgroup", o, t)
                    } else {
                        break
                    }
                }
                if (a || r) {
                    return new g(new c("supsub", {
                        base: i && i.result,
                        sup: a,
                        sub: r
                    }, t), h)
            ...
            o.prototype.parseImplicitGroup = function(e, t) {
                ...
                if (h === "\\left") {
                    ...
                    return new g(new c("leftright", {
                        body: r.result,
                        left: l.result.value.value,
                        right: p.result.value.value
                    }, t), p.position)
                } else if (h === "\\begin") {
                    var o = this.parseFunction(e, t);
                    var d = o.result.value.name;
                    if (!a.hasOwnProperty(d)) {
                        throw new n("No such environment: " + d, this.lexer, o.result.value.namepos)
                    }
                    var u = a[d];
                    var w = [null, t, d];
                    var f = this.parseArguments(o.position, t, "\\begin{" + d + "}", u, w);
                    w[0] = f;
                    var v = u.handler.apply(this, w);
                    var y = this.lexer.lex(v.position, t);
                    this.expect(y, "\\end");
                    var x = this.parseFunction(v.position, t);
                    if (x.result.value.name !== d) {
                        throw new n("Mismatch: \\begin{" + d + "} matched " + "by \\end{" + x.result.value.name + "}", this.lexer, x.namepos)
                    }
                    v.position = x.position;
                    return v
                } else if (s.contains(k, h)) {
                    r = this.parseExpression(i.result.position, t, false);
                    return new g(new c("sizing", {
                        size: "size" + (s.indexOf(k, h) + 1),
                        value: r.result
                    }, t), r.position)
                } else if (s.contains(m, h)) {
                    r = this.parseExpression(i.result.position, t, true);
                    return new g(new c("styling", {
            ...
            o.prototype.parseFunction = function(e, t) {
                ...
                if (i) {
                    if (i.isFunction) {
                        ...
                        if (t === "text" && !r.allowedInText) {
                            throw new n("Can't use function '" + a + "' in text mode", this.lexer, i.position)
                        }
                        var l = [a];
                        var s = this.parseArguments(i.result.position, t, a, r, l);
                        var p = h.funcs[a].handler.apply(this, l);
                        return new g(new c(p.type, p, t), s)
                    } else {
                        return i.result
                    }
                } else {
            ...
            o.prototype.parseArguments = function(e, t, i, a, r) {
                ...
                var o = [s];
                for (var c = 0; c < l; c++) {
                    var g = a.argTypes && a.argTypes[c];
                    var d;
                    if (c < a.numOptionalArgs) {
                        if (g) {
                            d = this.parseSpecialGroup(s, g, t, true)
                        } else {
                            d = this.parseOptionalGroup(s, t)
                        }
                        if (!d) {
                            r.push(null);
                            o.push(s);
                        ...
                        if (g) {
                            d = this.parseSpecialGroup(s, g, t)
                        } else {
                            d = this.parseGroup(s, t)
                        }
                        if (!d) {
                            throw new n("Expected group after '" + i + "'", this.lexer, s)
                        }
                    }
                    var u;
                    if (d.isFunction) {
                        var w = h.funcs[d.result.result].greediness;
                        if (w > p) {
                            u = this.parseFunction(s, t)
                        } else {
                            throw new n("Got function '" + d.result.result + "' as " + "argument to '" + i + "'", this.lexer, d.result.position - 1)
                        }
                    } else {
                        u = d.result
                    }
            ...
            o.prototype.parseSpecialGroup = function(e, t, i, h) {
                if (t === "original") {
                    t = i
                }
                if (t === "color" || t === "size") {
                    ...
                    this.expect(s, h ? "]" : "}");
                    return new d(new g(new c(t, l, i), s.position), false)
                } else if (t === "text") {
            ...
            o.prototype.parseGroup = function(e, t) {
            ...
            o.prototype.parseOptionalGroup = function(e, t) {
                var i = this.lexer.lex(e, t);
                if (i.text === "[") {
                    var h = this.parseExpression(i.position, t, false, "]");
                    var a = this.lexer.lex(h.position, t);
            ...
            o.prototype.parseSymbol = function(e, t) {
                var i = this.lexer.lex(e, t);
                if (h.funcs[i.text]) {
        ...
        8: [function(e, t, i) {
            ...
            var u = [new h(a, 0, 1, false), new h(r, 0, 1, true), new h(l, 1, 1, false), new h(s, 1, 1, true), new h(p, 2, .7, false), new h(n, 2, .7, true), new h(o, 3, .5, false), new h(c, 3, .5, true)];
            var w = [p, n, p, n, o, c, o, c];
            var k = [n, n, n, n, c, c, c, c];
            var m = [l, s, p, n, o, c, o, c];
            var f = [s, s, n, n, c, c, c, c];
            var v = [r, r, s, s, n, n, c, c];
            t.exports = {
                DISPLAY: u[a],
                TEXT: u[l],
                SCRIPT: u[p],
        ...
        9: [function(e, t, i) {
            ...
            var l = function(e, t, i, l, s) {
                ...
                var n;
                if (p) {
                    n = new h.symbolNode(e, p.height, p.depth, p.italic, p.skew, s)
                } else {
                    typeof console !== "undefined" && console.warn("No character metrics for '" + e + "' in style '" + t + "'");
                    n = new h.symbolNode(e, 0, 0, 0, 0, s)
                }
                if (l) {
                    n.style.color = l
                }
            ...
            var g = function(e, t) {
                var i = o([], [new h.symbolNode("\u200b")]);
                i.style.fontSize = t / e.style.sizeMultiplier + "em";
                var a = o(["fontsize-ensurer", "reset-" + e.size, "size5"], [i]);
                return a
            };
            var d = function(e, t, i, a) {
                ...
                if (t === "individualShift") {
                    ...
                    for (s = 1; s < p.length; s++) {
                        var n = -p[s].shift - l - p[s].elem.depth;
                        var c = n - (p[s - 1].elem.height + p[s - 1].elem.depth);
                        l = l + n;
                        e.push({
                            type: "kern",
                            size: c
                ...
                var y = o(["baseline-fix"], [w, new h.symbolNode("\u200b")]);
                k.push(y);
                var x = o(["vlist"], k);
        ...
        10: [function(e, t, i) {
            ...
            var f = function(e) {
                return c(["sizing", "reset-" + e.size, "size5", e.style.reset(), r.TEXT.cls(), "nulldelimiter"])
            };
            var v = {
                ...
                bin: function(e, t, i) {
                    ...
                    if (!i || o.contains(["mbin", "mopen", "mrel", "mop", "mpunct"], u(a))) {
                        e.type = "textord";
                        h = "mord"
                    }
                ...
                ordgroup: function(e, t, i) {
                    return c(["mord", t.style.cls()], g(e.value, t.reset()))
                },
                text: function(e, t, i) {
                    return c(["text", "mord", t.style.cls()], g(e.value.body, t.reset()))
                },
                ...
                supsub: function(e, t, i) {
                    ...
                    var a, s, o, g;
                    if (e.value.sup) {
                        o = y(e.value.sup, t.withStyle(t.style.sup()));
                        a = c([t.style.reset(), t.style.sup().cls()], [o])
                    }
                    if (e.value.sub) {
                        g = y(e.value.sub, t.withStyle(t.style.sub()));
                        s = c([t.style.reset(), t.style.sub().cls()], [g])
                    }
                    var d, k;
                    if (m(e.value.base)) {
                        d = 0;
                    ...
                    var x = r.TEXT.sizeMultiplier * t.style.sizeMultiplier;
                    var b = .5 / n.metrics.ptPerEm / x + "em";
                    var z;
                    if (!e.value.sup) {
                        k = Math.max(k, n.metrics.sub1, g.height - .8 * n.metrics.xHeight);
                        z = l.makeVList([{
                            type: "elem",
                            elem: s
                        }], "shift", k, t);
                        ...
                        d = Math.max(d, f, o.depth + .25 * n.metrics.xHeight);
                        z = l.makeVList([{
                            type: "elem",
                            elem: a
                        }], "shift", -d, t);
                        ...
                        var S = n.metrics.defaultRuleThickness;
                        if (d - o.depth - (g.height - k) < 4 * S) {
                            k = 4 * S - (d - o.depth) + g.height;
                            var M = .8 * n.metrics.xHeight - (d - o.depth);
                            if (M > 0) {
                                d += M;
                                k -= M
                            }
                ...
                genfrac: function(e, t, i) {
                    ...
                    var a = h.fracNum();
                    var p = h.fracDen();
                    var o = y(e.value.numer, t.withStyle(a));
                    var g = c([h.reset(), a.cls()], [o]);
                    var d = y(e.value.denom, t.withStyle(p));
                    var u = c([h.reset(), p.cls()], [d]);
                    var w;
                    ...
                    if (h.size === r.DISPLAY.size) {
                        ...
                        if (w > 0) {
                            ...
                            m = 7 * n.metrics.defaultRuleThickness
                        }
                        v = n.metrics.denom1
                    } else {
                        if (w > 0) {
                            ...
                            k = n.metrics.num3;
                            m = 3 * n.metrics.defaultRuleThickness
                        }
                        v = n.metrics.denom2
                    }
                    var x;
                    if (w === 0) {
                        var b = k - o.depth - (d.height - v);
                        if (b < m) {
                            k += .5 * (m - b);
                            v += .5 * (m - b)
                        }
                        ...
                        var z = n.metrics.axisHeight;
                        if (k - o.depth - (z + .5 * w) < m) {
                            k += m - (k - o.depth - (z + .5 * w))
                        }
                        if (z - .5 * w - (d.height - v) < m) {
                            v += m - (z - .5 * w - (d.height - v))
                        }
                        var S = c([t.style.reset(), r.TEXT.cls(), "frac-line"]);
                        S.height = w;
                        var M = -(z - .5 * w);
                        x = l.makeVList([{
                            type: "elem",
                    ...
                    if (h.size === r.DISPLAY.size) {
                        ...
                        q = n.metrics.getDelim2(h)
                    }
                    var A, T;
                    if (e.value.leftDelim == null) {
                        A = f(t)
                    } else {
                        A = s.customSizedDelim(e.value.leftDelim, q, true, t.withStyle(h), e.mode)
                    }
                    if (e.value.rightDelim == null) {
                        T = f(t)
                    } else {
                        T = s.customSizedDelim(e.value.rightDelim, q, true, t.withStyle(h), e.mode)
                    }
                    return c(["mord", t.style.reset(), h.cls()], [A, c(["mfrac"], [x]), T], t.getColor())
                },
                array: function(e, t, i) {
                    var h, a;
                    var r = e.value.body.length;
                    var s = 0;
                    var p = new Array(r);
                    var g = 1 / n.metrics.ptPerEm;
                    var d = 5 * g;
                    var u = 12 * g;
                    var w = o.deflt(e.value.arraystretch, 1);
                    var k = w * u;
                    var m = .7 * k;
                    var f = .3 * k;
                    var v = 0;
                    for (h = 0; h < e.value.body.length; ++h) {
                        var x = e.value.body[h];
                        var b = m;
                        var z = f;
                        if (s < x.length) {
                            s = x.length
                        ...
                        var q = 0;
                        if (e.value.rowGaps[h]) {
                            q = e.value.rowGaps[h].value;
                            switch (q.unit) {
                                case "em":
                                    q = q.number;
                                    break;
                                case "ex":
                                    q = q.number * n.metrics.emPerEx;
                                    break;
                                default:
                                    console.error("Can't handle unit " + q.unit);
                                    q = 0
                            }
                            if (q > 0) {
                                q += f;
                    ...
                    var A = v / 2 + n.metrics.axisHeight;
                    var T = e.value.cols || [];
                    var N = [];
                    var C;
                    for (a = 0; a < s; ++a) {
                        ...
                        P = l.makeVList(P, "individualShift", null, t);
                        P = c(["col-align-" + (E.align || "c")], [P]);
                        N.push(P);
                        if (a < s - 1 || e.value.hskipBeforeAndAfter) {
                            R = o.deflt(E.postgap, d);
                            if (R !== 0) {
                                C = c(["arraycolsep"], []);
                                C.style.width = R + "em";
                                N.push(C)
                    ...
                    p = c(["mtable"], N);
                    return c(["mord"], [p], t.getColor())
                },
                spacing: function(e, t, i) {
                    if (e.value === "\\ " || e.value === "\\space" || e.value === " " || e.value === "~") {
                        return c(["mord", "mspace"], [l.mathrm(e.value, e.mode)])
                    } else {
                        return c(["mord", "mspace", l.spacingFunctions[e.value].className])
                    }
                },
                llap: function(e, t, i) {
                    var h = c(["inner"], [y(e.value.body, t.reset())]);
                    var a = c(["fix"], []);
                    return c(["llap", t.style.cls()], [h, a])
                },
                rlap: function(e, t, i) {
                    var h = c(["inner"], [y(e.value.body, t.reset())]);
                    var a = c(["fix"], []);
                    return c(["rlap", t.style.cls()], [h, a])
                },
                op: function(e, t, i) {
                    var h;
                    var a;
                    ...
                    var p = ["\\smallint"];
                    var g = false;
                    if (t.style.size === r.DISPLAY.size && e.value.symbol && !o.contains(p, e.value.body)) {
                        g = true
                    }
                    var d;
                    var u = 0;
                    ...
                    if (e.value.symbol) {
                        ...
                        u = (d.height - d.depth) / 2 - n.metrics.axisHeight * t.style.sizeMultiplier;
                        w = d.italic
                    } else {
                        var m = [];
                    ...
                    if (s) {
                        ...
                        if (h) {
                            ...
                            v = c([t.style.reset(), t.style.sup().cls()], [S]);
                            x = Math.max(n.metrics.bigOpSpacing1, n.metrics.bigOpSpacing3 - S.depth)
                        }
                        if (a) {
                            var M = y(a, t.withStyle(t.style.sub()));
                            b = c([t.style.reset(), t.style.sub().cls()], [M]);
                            z = Math.max(n.metrics.bigOpSpacing2, n.metrics.bigOpSpacing4 - M.height)
                        }
                        var q, A, T;
                        if (!h) {
                            A = d.height - u;
                            ...
                            T = n.metrics.bigOpSpacing5 + b.height + b.depth + z + d.depth + u;
                            q = l.makeVList([{
                                type: "kern",
                                size: n.metrics.bigOpSpacing5
                            }, {
                ...
                katex: function(e, t, i) {
                    var h = c(["k"], [l.mathrm("K", e.mode)]);
                    var a = c(["a"], [l.mathrm("A", e.mode)]);
                    a.height = (a.height + .2) * .75;
                    a.depth = (a.height - .2) * .75;
                    var r = c(["t"], [l.mathrm("T", e.mode)]);
                    var s = c(["e"], [l.mathrm("E", e.mode)]);
                    s.height = s.height - .2155;
                    s.depth = s.depth + .2155;
                    var p = c(["x"], [l.mathrm("X", e.mode)]);
                    return c(["katex-logo"], [h, a, r, s, p], t.getColor())
                },
                overline: function(e, t, i) {
                    ...
                    var s = c([t.style.reset(), r.TEXT.cls(), "overline-line"]);
                    s.height = a;
                    s.maxFontSize = 1;
                    var p = l.makeVList([{
                        type: "elem",
                    ...
                    return c(["overline", "mord"], [p], t.getColor())
                },
                sqrt: function(e, t, i) {
                    var h = y(e.value.body, t.withStyle(t.style.cramp()));
                    var a = n.metrics.defaultRuleThickness / t.style.sizeMultiplier;
                    var p = c([t.style.reset(), r.TEXT.cls(), "sqrt-line"], [], t.getColor());
                    p.height = a;
                    p.maxFontSize = 1;
                    var o = a;
                    ...
                    var u = d + g + a;
                    var w = c(["sqrt-sign"], [s.customSizedDelim("\\surd", u, false, t, e.mode)], t.getColor());
                    ...
                    if (!e.value.index) {
                        return c(["sqrt", "mord"], [w, f])
                    } else {
                        var v = y(e.value.index, t.withStyle(r.SCRIPTSCRIPT));
                        var x = c([t.style.reset(), r.SCRIPTSCRIPT.cls()], [v]);
                        var b = Math.max(w.height, f.height);
                        var z = Math.max(w.depth, f.depth);
                        var S = .6 * (b - z);
                        var M = l.makeVList([{
                            type: "elem",
                            elem: x
                        ...
                        var q = c(["root"], [M]);
                        return c(["sqrt", "mord"], [q, w, f])
                    }
                },
                sizing: function(e, t, i) {
                    var h = g(e.value.value, t.withSize(e.value.size), i);
                    var a = c(["mord"], [c(["sizing", "reset-" + t.size, e.value.size, t.style.cls()], h)]);
                ...
                styling: function(e, t, i) {
                    ...
                    return c([t.style.reset(), a.cls()], l)
                },
                delimsizing: function(e, t, i) {
                    var h = e.value.value;
                    if (h === ".") {
                        return c([d[e.value.delimType]])
                    }
                    return c([d[e.value.delimType]], [s.sizedDelim(h, e.value.size, t, e.mode)])
                },
                leftright: function(e, t, i) {
                    ...
                    if (e.value.right === ".") {
                        ...
                        n = s.leftRightDelim(e.value.right, a, r, t, e.mode)
                    }
                    h.push(n);
                    return c(["minner", t.style.cls()], h, t.getColor())
                },
                rule: function(e, t, i) {
                    var h = c(["mord", "rule"], [], t.getColor());
                    var a = 0;
                    if (e.value.shift) {
                ...
                accent: function(e, t, i) {
                    ...
                    var s = y(h, t.withStyle(t.style.cramp()));
                    var p;
                    if (m(h)) {
                        var o = k(h);
                        var g = y(o, t.withStyle(t.style.cramp()));
                        p = g.skew
                    } else {
                        p = 0
                    }
                    var d = Math.min(s.height, n.metrics.xHeight);
                    var u = l.makeSymbol(e.value.accent, "Main-Regular", "math", t.getColor());
                    u.italic = 0;
                    var w = e.value.accent === "\\vec" ? "accent-vec" : null;
                    var f = c(["accent-body", w], [c([], [u])]);
                    f = l.makeVList([{
                        type: "elem",
                        elem: s
                    }, {
            ...
            var x = function(e, t) {
                ...
                var l = g(e, a);
                var s = c(["base", a.style.cls()], l);
                var p = c(["strut"]);
                var n = c(["strut", "bottom"]);
                p.style.height = s.height + "em";
                n.style.height = s.height + s.depth + "em";
                n.style.verticalAlign = -s.depth + "em";
                var o = c(["katex-html"], [p, n, s]);
        ...
        11: [function(e, t, i) {
            ...
            var n = {
                ...
                text: function(e) {
                    var t = o(e.value.body);
                    var i = new a.MathNode("mtext", t);
                    return i
                },
                color: function(e) {
                    var t = o(e.value.value);
                    var i = new a.MathNode("mstyle", t);
                    i.setAttribute("mathcolor", e.value.color);
                    return i
                },
                supsub: function(e) {
                    var t = [c(e.value.base)];
                    if (e.value.sub) {
                        t.push(c(e.value.sub))
                    }
                    if (e.value.sup) {
                        t.push(c(e.value.sup))
                    }
                    var i;
                    if (!e.value.sub) {
                        i = "msup"
                ...
                genfrac: function(e) {
                    var t = new a.MathNode("mfrac", [c(e.value.numer), c(e.value.denom)]);
                    if (!e.value.hasBarLine) {
                ...
                sqrt: function(e) {
                    var t;
                    if (e.value.index) {
                        t = new a.MathNode("mroot", [c(e.value.body), c(e.value.index)])
                    } else {
                        t = new a.MathNode("msqrt", [c(e.value.body)])
                    }
                    return t
                },
                ...
                accent: function(e) {
                    ...
                    var i = new a.MathNode("mover", [c(e.value.base), t]);
                ...
                overline: function(e) {
                    ...
                    var i = new a.MathNode("mover", [c(e.value.body), t]);
                    i.setAttribute("accent", "true");
                    return i
                },
                rule: function(e) {
                ...
                llap: function(e) {
                    var t = new a.MathNode("mpadded", [c(e.value.body)]);
            ...
            var g = function(e, t, i) {
                ...
                var n = new a.MathNode("math", [p]);
                return s(["katex-mathml"], [n])
            };
            t.exports = g
        }, {
            "./ParseError": 5,
        ...
        13: [function(e, t, i) {
            ...
            var d = function(e, t, i, h, a) {
                ...
                var p = g(s, t, h);
                if (i) {
                    var n = (1 - h.style.sizeMultiplier / t.sizeMultiplier) * l.metrics.axisHeight;
                    p.style.top = n + "em";
                    p.height -= n;
                    p.depth += n
                }
            ...
            var u = function(e, t, i, h, r) {
                var s = c(e, t, r);
                var p = g(n(["delimsizing", "size" + t], [s], h.getColor()), a.TEXT, h);
                if (i) {
                    var o = (1 - h.style.sizeMultiplier) * l.metrics.axisHeight;
                    p.style.top = o + "em";
                    p.height -= o;
                    p.depth += o
                }
            ...
            var w = function(e, t, i) {
                ...
                var a = n(["delimsizinginner", h], [n([], [r.makeSymbol(e, t, i)])]);
                return {
                    type: "elem",
                    elem: a
                }
            ...
            var k = function(e, t, i, h, s) {
                ...
                E.push(w(p, k, s));
                var P = r.makeVList(E, "bottom", C, h);
                return g(n(["delimsizing", "mult"], [P], h.getColor()), a.TEXT, h)
            };
            ...
            var T = function(e, t, i, h, a) {
                ...
                var n = Math.max(t - r, i + r);
                var o = Math.max(n / 500 * s, 2 * n - p);
                return A(e, o, true, h, a)
            };
            t.exports = {
                sizedDelim: x,
                customSizedDelim: A,
        ...
        15: [function(e, t, i) {
            ...
            function p(e, t, i, h) {
                var a = [],
                    ...
                    n = [];
                while (true) {
                    var o = e.parseExpression(t, i, false, null);
                    a.push(new l("ordgroup", o.result, i));
                    t = o.position;
                    var c = o.peek.text;
                    if (c === "&") {
                        t = o.peek.position
                        ...
                        n.push(g.result.value.size);
                        t = g.position;
                        a = [];
                        p.push(a)
                    } else {
                        throw new r("Expected & or \\\\ or \\end", e.lexer, o.peek.position)
                    }
                }
                h.body = p;
                h.rowGaps = n;
        ...
        16: [function(e, t, i) {
            ...
            var H = e("./fontMetricsData");
            var X = function(e, t) {
                return H[t][e.charCodeAt(0)]
            };
            t.exports = {
                metrics: V,
                getCharacterMetrics: X
            }
        }, {
            "./Style": 8,
            "./fontMetricsData": 17
